...
plot_map<-function(fil,chrN,title_name,xmin_all,xmax_all,xaxis_mark){
...
plot_chrom_map <- function(cnv_table, chr_lengths, plot_options = NULL) {
  ...
  if (ymin==-1){
    ...
    ymin_new <- floor(ymin/5)*5
  }
  print(c(ymin,ymax,ymin_new,ymax_new))
  ...
  # Sort out the y-axis
  y_axis_limits <- round(2*c(ymin_new,ymax_new), -1)/2
  ...
  xmin_all1 <- (round(xmin_all/10000000)-1)*10000000
  xmax_all1 <- (round(xmax_all/10000000)+1)*10000000
  print(paste(xmin_all1,xmax_all1))
  x_axis_major_ticks <- seq(from=xmin_all1,to=xmax_all1,by=10000000)
  if(xaxis_mark=="Xaxis"){
    axis(1, at=x_axis_major_ticks,labels = x_axis_major_ticks/1000000)
  }
  else{
    axis(1, at=x_axis_major_ticks,labels = NA)
  }
...
######################################Test for specicfic chromosome
# opt_file<-"All_test.pdf"
# pdf(opt_file,width=10,height=6)
# par(mfrow = c(2, 1), mar = c(2,3,1,2), oma = c(0,0,1,0))
# chr_i<-12
...
# plot_map("Anc_chrom_map.csv",chr_i,chr_i_mark,xmin_all,xmax_all,"No_Xaxis")
# if(any(df_all[,.(CHROM)]==chr_i)){
#   plot_map("All_events_without_wrong_chrom_map.csv",chr_i,"",xmin_all,xmax_all,"Xaxis")
# }else{
...
#"All_39chrs_chrom_map_with_Anc.pdf"
...
plot_map_X<-function(fil,chrN,title_name,xmin_all,xmax_all,xaxis_mark){
  ...
  plot_chrom_map <- function(cnv_table, chr_lengths, plot_options = NULL) {
    ...
    if("xlim"%in% names(plot_options)){
      dims[["xlim"]] <- plot_options[["xlim"]]
    }
    if (ymax==1){
      ymax_new<-5 
    }else{
...
# df_anc<-fread("Anc_chrom_map.csv")
# 
# opt_file<-"All_test_X.pdf"
# pdf(opt_file,width=10,height=6)
# par(mfrow = c(2, 1), mar = c(2,3,1,2), oma = c(0,0,1,0))
chr_i<-"X"

ll0<-get_dims_X("Anc_chrom_map.csv",chr_i)
ll1<-get_dims_X("All_events_without_wrong_chrom_map.csv",chr_i)

xmin_all<-min(c(ll1$xlim,ll0$xlim))
xmax_all<-max(c(ll1$xlim,ll0$xlim))

chr_i_mark<-paste("Chr",as.character(chr_i),sep="")
plot_map_X("Anc_chrom_map.csv",chr_i,chr_i_mark,xmin_all,xmax_all,"No")
if(any(df_all[,.(CHROM)]==chr_i)){
  plot_map_X("All_events_without_wrong_chrom_map.csv",chr_i,"",xmin_all,xmax_all,"Xaxis")
}else{
  plot(x=10,y=10,pch=".",col="white",xaxt="n",yaxt="n",axes=FALSE)
  text(x=10,y=10,"No Change for Ancestor")
}
